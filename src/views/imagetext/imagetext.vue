  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column
        ...
        align="center"
      />
      <el-table-column show-overflow-tooltip prop="images" label="图片" width="200" align="center"
      ...
      >
        <template v-slot="scope">
          ...
          <el-button type="primary" @click="wzfx(scope.row.articleId)">图文分析</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
import Dialog from '@/views/imagetext/components/dialog.vue'
import Wen from '@/views/imagetext/components/wen.vue'
...
const initArticleList = async () => {
  try {
    const res = await requestUtil.post('/api/articleImages/page', queryForm.value)
...
const handleDialogValue = (id) => {
  ...
  dialogTitle.value = id ? '图文编辑' : '图文新增'
...
const handleDelete = async (articleId) => {
  var ids = []
  if (articleId) {
    const res = await requestUtil.delete(`/api/articleImages/delete?id=${articleId}`)
    if (res.data.code == 0) {
      ElMessage({
        type: 'success',
        message: '执行成功'
    ...
    const res = await requestUtil.post('/api/articleImages/deleteBatch', ids)
    if (res.data.code == 0) {
      ElMessage({
        type: 'success',
        message: '执行成功'
...
const wzfx = async (articleId) => {
  ...
  const res = await requestUtil.get(`/api/articleImages/analysis?articleId=${longId.toString()}`)
