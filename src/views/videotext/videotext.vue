  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column
        ...
        align="center"
      />
      <el-table-column show-overflow-tooltip prop="images" label="视频" width="200" align="center"
      ...
      >
        <template v-slot="scope">
          ...
          <el-button type="primary" @click="wzfx(scope.row.articleId)">视频分析</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
import Dialog from '@/views/videotext/components/dialog.vue'
import Wen from '@/views/videotext/components/wen.vue'
...
const initArticleList = async () => {
  try {
    const res = await requestUtil.post('/api/video/page', queryForm.value)
...
const handleDialogValue = (id) => {
  ...
  dialogTitle.value = id ? '视频编辑' : '视频新增'
...
const handleDelete = async (articleId) => {
  var ids = []
  if (articleId) {
    const res = await requestUtil.delete(`/api/video/delete?id=${articleId}`)
    if (res.data.code == 0) {
      ElMessage({
        type: 'success',
        message: '执行成功'
    ...
    const res = await requestUtil.post('/api/video/deleteBatch', ids)
    if (res.data.code == 0) {
      ElMessage({
        type: 'success',
        message: '执行成功'
...
const wzfx = async (articleId) => {
  ...
  const res = await requestUtil.get(`/api/video/analysis?articleId=${longId.toString()}`)
  ...
  // 跳转到分析页面，并传递来源路径作为参数
  router.push({ path: '/Home/index', query: { from: fromPath } })
}
